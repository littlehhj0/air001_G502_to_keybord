...
HardwareTimer *key_tic = new HardwareTimer(TIM3);  // TIM3 is MCU hardware peripheral instance, its definition is provided in CMSIS
...
void Skey::begin(uint8_t sw, uint8_t sa, uint8_t sb, void (*func)(ec11_task_result_type task_type, int16_t rusult_value))
{
    ...
    key_tic->setOverflow(1000, HERTZ_FORMAT);   //1Hz
    key_tic->attachInterrupt(key_timer);
    key_tic->resume();
}
